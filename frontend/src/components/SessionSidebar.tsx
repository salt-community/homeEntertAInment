      ...
      <div className="flex-1 overflow-y-auto p-4">
        {sessions.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-400 text-xs">Create your first session to get started</p>
          </div>
        ) : (
            ...
            {sessions.map((session) => (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <h4 className="font-semibold text-gray-800 text-sm">{session.gameName}</h4>
                  <div className={`w-2 h-2 rounded-full ${session.isActive ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                </div>
                <div className="space-y-1">
                  <p className="text-xs text-gray-500">ID: {session.sessionId.slice(0, 12)}...</p>
                  <p className="text-xs text-gray-500">Created: {new Date(session.createdAt).toLocaleDateString()}</p>
                  {session.players && session.players.length > 0 && (
                    <p className="text-xs text-gray-500">{session.players.length} player(s)</p>
                  )}
                </div>
              </div>
            ))}
